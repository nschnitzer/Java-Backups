...
public class BinarySearcher 
{
	...
	{
		arr1 = array;
		sorted = BinarySearcher.Sort(arr1);
		pos = (sorted.length-1)/2;
	}
	
	//Sorts the array using the insertion sort algorithm
	public static int[] Sort(ArrayList<Integer> array1)
	{
	        for (int j = 1; j < array1.length(); j++) {  
	            int key = array1.get(j);  
	            int i = j-1;  
	            while ( (i > -1) && ( array1.get(i) > key ) ) {  
	                array1.set(i+1, i);
	                i--;  
	            }  
	            array1.set(i+1, key);
	...
	{
		array = BinarySearcher.selectionSort(array);
		...
		{
			return BinarySearcher.binarySearch(array, find, x, ((x+y)/2)-1);
		}
		else //If it is more than the mid element
		{
			return BinarySearcher.binarySearch(array, find, ((x+y)/2)+1, y);
		}
